...
You receive:
- rawText / reconstructedText: full OCR output — use this as the primary source of truth.
...
- rulesExtracted: fields already extracted by regex rules — use as hints, but verify against rawText.
...
- vendorName — UNIVERSAL (any invoice type):
  ...
  * Venezuela: "Razón Social" in the vendor block = vendorName. In Venezuela invoices, the vendor section comes before "Datos del Cliente" or "Receptor".
  * NEVER use document types, government agency names, or terminal IDs as vendor.
- customerName: check labeledFields for: Cliente, Receptor, Comprador, Adquiriente. For Venezuelan invoices, "Razón Social" in the customer/receptor section = customerName. Retail tickets: null.
- vendorIdentifications: RUC/RFC/NIT/CUIT/RIF/CIF from vendor section (before customer boundary). Venezuela: RIF J-XXXXXXX-X format.
- customerIdentifications: same ID types but from customer section only. For Venezuela, RIF of the buyer.
